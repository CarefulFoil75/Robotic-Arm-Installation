...
"""
File name:      neural_network_demo.py
Author:         Michael Johnson
Date:           3/20/2025
Description:    AI robot system neural network object and methods for loading and saving data and models for demo.
...
if __name__ == '__main__':
    times = []
    times_codebook = ['Making NN object', 'Loading data', 'Creating train-test splits', 'Saving data', 'Making model',
                      'Training model', 'Saving model', 'Testing model', 'Predicting and printing samples']

    start_time = time.time() # Start timing process
    demo_nn = AIRobotNeuralNetwork(cc.user_path)
    times.append(time.time()-start_time) # Log process end time

    # c = input("Load new data? Y/n: ")
    c = 'y'
    if c.lower() == 'y':
        start_time = time.time()  # Start timing process
        demo_nn.load_new_imgs()
        times.append(time.time() - start_time)  # Log process end time

        start_time = time.time()  # Start timing process
        demo_nn.create_train_test_sets()
        times.append(time.time() - start_time)  # Log process end time
    ...
    print("Loaded Images:")
    print(demo_nn.dataset.shape)
    print("Loaded Labels:")
    print(demo_nn.labels.shape)
    print("Loaded Dictionary:")
    print(demo_nn.block_dict.items())
    print(f"Training with {demo_nn.train_set.shape[0]} images.")
    ...
    colors = list(demo_nn.block_dict.keys())
    start_time = time.time()  # Start timing process
    predictions = demo_nn.make_predictions()

    print("Prediction 0 Confidence: ", predictions[0])
    print("Prediction: ", colors[np.argmax(predictions[0])])
    print("Answer: ", colors[demo_nn.test_labels[0][0]])
    ...
    for i in range(num_images):
        ...
        plot_image(i, predictions[i], demo_nn.test_labels, demo_nn.block_dict, demo_nn.test_set)
        plt.subplot(num_rows, 2 * num_cols, 2 * i + 2)
        plot_value_array(i, predictions[i], demo_nn.test_labels)
    plt.tight_layout()
    plt.show()
    times.append(time.time() - start_time)  # Log process end time

    for i, t in enumerate(times):
        print(f"{times_codebook[i]}: {t} seconds")
