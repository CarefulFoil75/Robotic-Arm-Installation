...
class AIRobotNeuralNetwork:
    ...
    def predict_from_live_feed(self):
        """
        Capture frames from the camera, preprocess, and make predictions in real-time.
        """
        cap = cv2.VideoCapture(0)  # Use the default camera (0), change if needed.
        ...
        while True:
            ...
            img = cv2.resize(frame, (640, 480))  # Resize to match model input
            ...
            label_name = list(self.block_dict.keys())[predicted_label]
            confidence = np.max(prediction[0]) * 100

            # Display prediction on the frame
            cv2.putText(frame, f"Prediction: {label_name} ({confidence:.2f}%)",
...
if __name__ == '__main__':
    ...
    # Load data or create new model as per previous logic
    ...
    # Start real-time camera feed prediction
    print("Starting live camera feed...")
    dev_nn.predict_from_live_feed()
